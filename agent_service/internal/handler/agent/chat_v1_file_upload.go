...
func (c *ControllerV1) runFileUpload(ctx context.Context, gctx *gin.Context) (res *dto.FileUploadRes, err error) {
	// 从请求中获取上传的文件
	...
	// 确保保存目录存在
	if err := os.MkdirAll(knowledge.FileDir, 0755); err != nil {
		return nil, fmt.Errorf("创建目录失败: %s: %w", knowledge.FileDir, err)
	}
	...
	// 保存文件
	savePath := filepath.Join(knowledge.FileDir, newFileName)
