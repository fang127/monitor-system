...
- `agent_service` 的记忆表按租户、团队、集群和会话组织，并且当前代码在缺少租户或团队时会返回错误；但聊天请求仍可以直接传入 `TenantId`、`TeamId`，这些字段还没有和登录身份绑定。
...
- 保持 `agent_service` 对缺少租户或团队的拒绝行为，要求部署和登录链路显式提供租户、团队和集群归属。
...
- 不在本次变更中设计历史数据默认归属迁移；已有历史数据如需迁移，必须由运维明确指定目标租户、团队和集群。
...
JWT 中包含 `user_id`、`username`、`role`、`tenant_id`、`team_id`。当用户属于多个团队时，登录必须选择一个已授权团队，后续通过切换团队接口重新签发当前作用域 JWT。
...
基础身份脚本只创建表结构，不自动创建默认租户或默认团队。部署或管理流程必须显式创建租户、团队和成员关系；没有可用团队成员关系的用户不能获得可访问业务数据的 JWT。
...
短期可以在所有监控事实表增加 `tenant_id`、`team_id`、`cluster_id` 字段并补索引；长期推荐用 `server_id` 关联 `servers` 表，减少明细表重复维度。考虑当前查询 SQL 大量直接按 `server_name` 过滤，第一阶段可采用显式作用域字段方案，后续再演进到 `server_id` 主索引。
...
`agent_service` 已经在 `ValidateMemoryScope` 和 `ValidateSessionScope` 中要求租户、团队和会话。新设计沿用这个边界：

- `api_gateway` 登录时必须选择已授权团队并签发带 `tenant_id/team_id` 的 JWT。
- worker 注册时必须绑定明确的租户、团队和集群。
- manager 写入监控事实前必须解析到明确作用域；解析失败时拒绝写入。
- 前端调用 AI 对话时不再伪造租户/团队，而是依赖 JWT，集群由当前团队下的集群选择器提供。

## Risks / Trade-offs

- 作用域字段迁移范围大 → 分阶段落地，先补身份和资产表，再补查询过滤，最后收紧请求体信任边界。
- 旧数据没有明确作用域时无法自动归属 → 不做默认猜测，要求运维提供显式映射后再迁移。
...
1. 新增身份和资产 schema，但不自动创建默认租户、团队或集群。
2. 管理员或部署流程显式创建租户、团队、成员关系、集群、服务器和 worker 注册记录。
3. 扩展 JWT、认证中间件和 `agent_service` claims 解析，缺少租户或团队时继续拒绝业务访问。
4. 如需迁移历史用户或监控数据，先由运维提供明确目标作用域映射，再执行定制迁移脚本。
...
回滚策略：保留旧查询路径；若新作用域过滤出现问题，可临时禁用多租户过滤，但不得自动把缺少作用域的数据归入某个默认租户，也不得回滚已签发 token 的签名密钥和用户密码数据。
