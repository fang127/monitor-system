...
当前系统已经在 `agent_service` 记忆系统中引入租户、团队、集群和会话作用域，并且缺少租户或团队时会拒绝请求，但登录用户、JWT、worker 上报数据和监控查询链路没有共享同一套作用域模型。这样会导致前端和后端身份无法自然提供 agent 记忆所需作用域、用户可通过请求体伪造作用域、监控事实无法按租户和团队隔离，也会让后续多租户部署缺少清晰边界。
...
- 部署约束：租户、团队、集群和成员关系必须显式创建；缺少租户或团队的 AI 记忆请求继续按现有行为报错。
