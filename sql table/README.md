...
| 脚本                          | 归属模块                     | 说明                                                         |
| ----------------------------- | ---------------------------- | ------------------------------------------------------------ |
| `init_server_performance.sql` | manager、api_gateway、worker | 创建监控主库、登录用户表、服务器性能汇总表和各类指标明细表。 |
...
| `agent_session_memories`   | `agent_service` | `agent_service` | 保存会话摘要和最近消息窗口，用于连续对话上下文。    |
| `agent_long_term_memories` | `agent_service` | `agent_service` | 保存长期记忆正文、来源、置信度、状态和向量索引 ID。 |
| `agent_memory_events`      | `agent_service` | `agent_service` | 记录记忆创建、召回、更新、删除等治理事件。          |
