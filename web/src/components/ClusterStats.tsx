  ...
  return (
    <div className="stats-grid">
      ...
      <StatCard
        ...
        tone={data.offline_servers > 0 ? "danger" : "success"}
        helper="状态 OFFLINE"
      />
      <StatCard
        label="平均分"
      ...
      <StatCard
        label="最高分"
        value={formatScore(data.max_score)}
        tone="success"
        helper={data.best_server || "最佳服务器"}
      />
      <StatCard
        label="最低分"
        value={formatScore(data.min_score)}
        tone={data.min_score >= 60 ? "warning" : "danger"}
        helper={data.worst_server || "最低服务器"}
      />
    </div>
  );
}
