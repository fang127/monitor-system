...
export function AnomaliesPage() {
  ...
  const [state, setState] = useState<AsyncState<QueryAnomalyResponse>>({
    data: null,
    loading: true,
    error: null,
  });
  ...
  return (
    <div className="page-stack">
      <header className="page-header">
        <div>
          <span className="eyebrow">GET /api/servers/:server/anomalies</span>
          <h1>{serverName} 异常记录</h1>
        </div>
      </header>
      <QueryControls
        onApply={(params) => {
        ...
        extra={
          <>
            ...
            <label>
              ...
              <input
                value={memThreshold}
                onChange={(event) => setMemThreshold(event.target.value)}
                inputMode="decimal"
              />
            </label>
            <label>
      ...
      <section className="section-block">
        ...
        {state.error ? (
          <ErrorState title="异常记录" message={state.error} />
        ) : null}
