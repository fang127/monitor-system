...
function normalizeKind(value: string | undefined): DetailKind {
  return detailKinds.includes(value as DetailKind)
    ? (value as DetailKind)
    : "net";
}
  ...
  return [
    {
      key: "timestamp",
      title: "时间",
      render: (row) => formatDateTime(String(row.timestamp)),
    },
  ...
  net: {
    ...
    columns: [
      ...commonColumns("net_name", "网卡"),
      {
        ...
        title: "接收速率",
        render: (row) => {
          const unit = pickBytesRateUnit([
            Number(row.rcv_bytes_rate),
            Number(row.snd_bytes_rate),
          ]);
      ...
      {
        key: "rcv_packets_rate",
        title: "收包速率",
        render: (row) => formatNumber(Number(row.rcv_packets_rate), 1),
      },
      {
        key: "snd_packets_rate",
        title: "发包速率",
        render: (row) => formatNumber(Number(row.snd_packets_rate), 1),
      },
  ...
  disk: {
    ...
    columns: [
      ...commonColumns("disk_name", "磁盘"),
      {
        ...
        title: "读速率",
        render: (row) => formatBytesRate(Number(row.read_bytes_per_sec)),
      },
      {
        key: "write_bytes_per_sec",
        title: "写速率",
        render: (row) => formatBytesRate(Number(row.write_bytes_per_sec)),
      },
      {
        key: "read_iops",
        title: "读 IOPS",
        render: (row) => formatNumber(Number(row.read_iops), 1),
      },
      {
        key: "write_iops",
        title: "写 IOPS",
        render: (row) => formatNumber(Number(row.write_iops), 1),
      },
      {
        key: "util_percent",
        title: "利用率",
        render: (row) => formatPercent(Number(row.util_percent)),
      },
    ],
    chartSeries: [
  ...
  mem: {
    ...
    columns: [
      ...
      },
      {
        key: "total",
        title: "总量 MB",
        render: (row) => formatNumber(Number(row.total), 1),
      },
      {
        key: "free",
        title: "空闲 MB",
        render: (row) => formatNumber(Number(row.free), 1),
      },
      {
        key: "avail",
        title: "可用 MB",
        render: (row) => formatNumber(Number(row.avail), 1),
      },
      {
        key: "cached",
        title: "Cached MB",
        render: (row) => formatNumber(Number(row.cached), 1),
      },
      {
        key: "active",
        title: "Active MB",
        render: (row) => formatNumber(Number(row.active), 1),
      },
    ],
    chartSeries: [
      { key: "free", name: "Free" },
  ...
  softirq: {
    ...
    columns: [
      ...
      { key: "rcu", title: "RCU" },
      {
        key: "net_rx_rate",
        title: "NET_RX变化率",
        render: (row) => formatNumber(Number(row.net_rx_rate), 2),
      },
    ],
    chartSeries: [
  ...
  mysql: {
    ...
    columns: [
      ...commonColumns("instance", "实例"),
      {
        ...
        render: (row) => (
          <span
            className={`status-pill ${row.up ? "status-online" : "status-critical"}`}
          >
            {row.up ? "正常" : "不可用"}
          </span>
        ),
      ...
      },
      {
        key: "qps",
        title: "QPS",
        render: (row) => formatNumber(Number(row.qps), 1),
      },
      {
        key: "tps",
        title: "TPS",
        render: (row) => formatNumber(Number(row.tps), 1),
      },
      {
        key: "slow_queries_rate",
        title: "慢查询/s",
        render: (row) => formatNumber(Number(row.slow_queries_rate), 2),
      },
      {
        key: "innodb_row_lock_waits_rate",
        title: "锁等待/s",
        render: (row) =>
          formatNumber(Number(row.innodb_row_lock_waits_rate), 2),
      },
      {
        key: "innodb_buffer_pool_hit_percent",
        title: "Buffer Pool",
        render: (row) =>
          formatPercent(Number(row.innodb_buffer_pool_hit_percent)),
      },
      {
        key: "replication_lag_seconds",
        title: "复制延迟",
        render: (row) =>
          row.replication_configured
            ? `${formatNumber(Number(row.replication_lag_seconds), 1)}s`
            : "--",
      },
    ],
    chartSeries: [
  ...
  redis: {
    ...
    columns: [
      ...
      },
      {
        ...
        title: "内存压力",
        render: (row) =>
          Number(row.maxmemory) > 0
            ? formatPercent(Number(row.memory_used_percent))
            : "--",
      },
      {
        key: "commands_per_sec",
        title: "命令/s",
        render: (row) => formatNumber(Number(row.commands_per_sec), 1),
      },
      {
        key: "keyspace_hit_percent",
        title: "命中率",
        render: (row) =>
          Number(row.keyspace_hits) + Number(row.keyspace_misses) > 0
            ? formatPercent(Number(row.keyspace_hit_percent))
            : "--",
      },
      ...
      { key: "rejected_connections", title: "拒绝连接" },
      {
        ...
        render: (row) =>
          row.replication_configured
            ? `${formatNumber(Number(row.master_last_io_seconds_ago), 1)}s`
            : "--",
      },
      {
        key: "slowlog_growth",
        title: "慢日志增长",
        render: (row) => formatNumber(Number(row.slowlog_growth), 1),
      },
    ],
    chartSeries: [
      { key: "connection_used_percent", name: "连接压力" },
...
export function DetailPage() {
  ...
  const [state, setState] = useState<AsyncState<DetailState>>({
    data: null,
    loading: true,
    error: null,
  });
  ...
  return (
    <div className="page-stack">
      <header className="page-header">
        <div>
          <span className="eyebrow">{`GET /api/servers/:server/${config.endpoint}`}</span>
          <h1>
      ...
      <section className="section-block">
        ...
        {state.error ? (
          <ErrorState title={config.title} message={state.error} />
        ) : null}
        {!state.loading && !state.error ? (
          <>
            <MetricLineChart<MetricRow>
              title={`${config.title}趋势`}
              rows={rows}
