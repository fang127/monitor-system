...
export function PerformancePage() {
  ...
  const [state, setState] = useState<AsyncState<QueryPerformanceResponse>>({ data: null, loading: true, error: null });
  ...
  return (
    <div className="page-stack">
      <header className="page-header">
        <div><span className="eyebrow">GET /api/servers/:server/performance</span><h1>{serverName} 历史性能</h1></div>
      </header>
      <QueryControls onApply={(params) => { setPage(1); setTimeRange(params); }} />
      <section className="section-block">
        {state.loading ? <LoadingState title="历史性能" /> : null}
        {state.error ? <ErrorState title="历史性能" message={state.error} /> : null}
        {!state.loading && !state.error ? (
          <>
            ...
            />
            <DataTable rows={state.data?.records || []} columns={columns} emptyTitle="历史性能" />
