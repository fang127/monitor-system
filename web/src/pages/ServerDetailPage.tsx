...
function findSummary(
  latest: QueryLatestScoreResponse | null,
  server: string,
): ServerScoreSummary | null {
...
export function ServerDetailPage() {
  ...
  const [state, setState] = useState<AsyncState<DetailBundle>>({
    data: null,
    loading: true,
    error: null,
  });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([latest, performance, trend, anomalies]) => {
        if (active) {
          setState({
            data: { latest, performance, trend, anomalies },
            loading: false,
            error: null,
          });
        }
  ...
  const summary = useMemo(
    () => findSummary(state.data?.latest || null, serverName),
    [serverName, state.data],
  );
  ...
  return (
    <div className="page-stack">
      <header className="page-header">
        ...
        <div className="button-group">
          <Link
            ...
            to={`/servers/${encodeURIComponent(serverName)}/performance`}
          >
            性能
          </Link>
          <Link
            className="button-link"
            to={`/servers/${encodeURIComponent(serverName)}/trend`}
          >
            趋势
          </Link>
          <Link
            className="button-link"
            to={`/servers/${encodeURIComponent(serverName)}/anomalies`}
          >
            异常
          </Link>
        </div>
      ...
      {state.error ? (
        <ErrorState title="服务器详情" message={state.error} />
      ) : null}
      {!state.loading && !state.error ? (
        <>
            ...
            <div className="summary-grid">
              ...
              <div className="stats-grid stats-grid-compact">
                <StatCard
                  ...
                  tone={summary ? scoreTone(summary.score) : "neutral"}
                />
                <StatCard
                  label="CPU"
                  value={formatPercent(summary?.cpu_percent)}
                />
                <StatCard
                  label="内存"
                  value={formatPercent(summary?.mem_used_percent)}
                />
                <StatCard
                  label="磁盘"
                  value={formatPercent(summary?.disk_util_percent)}
                />
                <StatCard
                  label="Load 1m"
                  value={summary ? summary.load_avg_1.toFixed(2) : "--"}
                />
                <StatCard
                  label="最后更新"
                  value={formatDateTime(summary?.last_update)}
                />
              </div>
            </div>
          </section>
          ...
          <div className="dashboard-grid">
            ...
            <section className="section-block">
              ...
              <div className="detail-link-grid">
                <Link
                  to={`/servers/${encodeURIComponent(serverName)}/details/net`}
                >
                  网络明细
                </Link>
                <Link
                  to={`/servers/${encodeURIComponent(serverName)}/details/disk`}
                >
                  磁盘明细
                </Link>
                <Link
                  to={`/servers/${encodeURIComponent(serverName)}/details/mem`}
                >
                  内存明细
                </Link>
                <Link
                  to={`/servers/${encodeURIComponent(serverName)}/details/softirq`}
                >
                  软中断明细
                </Link>
                <Link
                  to={`/servers/${encodeURIComponent(serverName)}/details/mysql`}
                >
                  MySQL 明细
                </Link>
                <Link
                  to={`/servers/${encodeURIComponent(serverName)}/details/redis`}
                >
                  Redis 明细
                </Link>
              </div>
          ...
          <section className="section-block">
            ...
            <DataTable
              rows={state.data?.anomalies.anomalies || []}
