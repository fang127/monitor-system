...
import type { AsyncState, QueryScoreRankResponse, SortOrder } from '../types/api';
...
export function ServersPage() {
  ...
  const [rank, setRank] = useState<AsyncState<QueryScoreRankResponse>>(initialState);
    ...
    <div className="page-stack">
      <header className="page-header">
        ...
        <label className="inline-control">
          排序
          <select value={order} onChange={(event) => { setPage(1); setOrder(event.target.value as SortOrder); }}>
      ...
      <section className="section-block">
        ...
        {rank.error ? <ErrorState title="评分列表" message={rank.error} /> : null}
        {!rank.loading && !rank.error ? <ServerTable data={rank.data?.servers || []} /> : null}
