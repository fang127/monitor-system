...
const columns: Column<PerformanceRecord>[] = [
  ...
  {
    key: "cpu_percent_rate",
    title: "CPU变化率",
    render: (row) => formatNumber(row.cpu_percent_rate, 3),
  },
  {
...
export function TrendPage() {
  ...
  const [state, setState] = useState<AsyncState<QueryTrendResponse>>({
    data: null,
    loading: true,
    error: null,
  });
  ...
  return (
    <div className="page-stack">
      <header className="page-header">
        <div>
          <span className="eyebrow">GET /api/servers/:server/trend</span>
          <h1>{serverName} 趋势</h1>
        </div>
      </header>
      <QueryControls
      ...
      <section className="section-block">
        ...
        {state.error ? (
          <ErrorState title="趋势数据" message={state.error} />
        ) : null}
