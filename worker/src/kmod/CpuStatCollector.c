 ...
 * - 状态包括：user, nice, system, idle, iowait, irq, softirq, steal, guest,
 * guest_nice
 */
...
#define DEVICE_NAME "CpuStatCollector"
#define CLASS_NAME  "CpuStatCollector"
#define MAX_CPUS    256

/* 将纳秒转换为 jiffies（clock_t）- 内联实现 */
// jiffies是Linux内核维护的全局计数器，以HZ为频率递增，记录系统启动后的时钟中断次数。
...
static struct cdev cpu_stat_cdev;      // 字符设备结构体
static struct class *cpu_stat_class;   // 设备类
static struct device *cpu_stat_device; // 设备结构体
...
static void update_cpu_stats(void)
{
    ...
    for_each_possible_cpu(cpu)
    {
        ...
        snprintf(cpu_stat_data[idx].cpu_name,
                 sizeof(cpu_stat_data[idx].cpu_name), "cpu%d", cpu);
        ...
        cpu_stat_data[idx].system =
            nsec_to_jiffies(kcs->cpustat[CPUTIME_SYSTEM]);
        ...
        cpu_stat_data[idx].softirq =
            nsec_to_jiffies(kcs->cpustat[CPUTIME_SOFTIRQ]);
        ...
        cpu_stat_data[idx].guest_nice =
            nsec_to_jiffies(kcs->cpustat[CPUTIME_GUEST_NICE]);

        idx++;
    }

    /* 标记结束 */
    if (idx < MAX_CPUS) cpu_stat_data[idx].cpu_name[0] = '\0';
}
