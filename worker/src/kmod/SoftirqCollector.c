...
#define DEVICE_NAME "SoftirqCollector"
#define CLASS_NAME  "SoftirqCollector"
#define MAX_CPUS    256
...
static void update_softirq_stats(void)
{
    ...
    for_each_possible_cpu(cpu)
    {
        ...
        snprintf(softirq_data[idx].cpu_name, sizeof(softirq_data[idx].cpu_name),
                 "cpu%d", cpu);
    ...
    /* 标记结束 */
    if (idx < MAX_CPUS) softirq_data[idx].cpu_name[0] = '\0';
}
